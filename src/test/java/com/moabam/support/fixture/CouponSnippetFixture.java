...
public final class CouponSnippetFixture {
	...
	public static final Snippet COUPON_STATUS_REQUEST = requestFields(
		fieldWithPath("ongoing").type(BOOLEAN).description("진행 상태 쿠폰 (true, false)"),
		fieldWithPath("notStarted").type(BOOLEAN).description("시작전 상태 쿠폰 (true, false)"),
		fieldWithPath("ended").type(BOOLEAN).description("종료 상태 쿠폰 (true, false)")
	);
